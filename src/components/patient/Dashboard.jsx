...
export default function PatientDashboard() {
  ...
  const [vitals, setVitals] = useState([
    ...
    { label: "Blood Sugar", value: "--", unit: "mg/dL" },
    ...
    const fetchVitals = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:5000/api/patient/getVitals/${phoneNumber}`,
        );
          ...
          setVitals([
            {
              ...
              value: data.vitals.blood_pressure
                ? `${data.vitals.blood_pressure.systolic}/${data.vitals.blood_pressure.diastolic}`
                : "--/--",
              unit: "mmHg",
            },
            {
              label: "Blood Sugar",
              value: data.vitals.blood_sugar
                ? data.vitals.blood_sugar.level
                : "--",
              unit: "mg/dL",
            },
            {
              label: "Weight",
              value: data.vitals.weight ? data.vitals.weight.value : "--",
              unit: "kg",
            },
          ]);
        }
  ...
  return (
    <div className="min-h-screen bg-[hsl(214,100%,97%)]">
      ...
      <div className="px-6 py-8 max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-[hsl(222,47%,11%)]">
            Welcome back!
          </h1>
          <p className="text-[hsl(215,16%,47%)] mt-1">
            Your health records are secure and ready
          </p>
        </div>
          ...
          {quickActions.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                href={item.to}
                className="bg-white rounded-2xl shadow-sm border border-[hsl(214,32%,91%)] p-8 flex flex-col items-center justify-center hover:shadow-md transition-all duration-300 no-underline"
              >
                <Icon className="w-10 h-10 text-[hsl(221,83%,53%)] mb-4" />
                <span className="text-sm font-semibold text-[hsl(222,47%,11%)]">
                  {item.label}
                </span>
              </Link>
            );
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[hsl(214,32%,91%)] p-8 mb-8">
          <div className="flex items-center gap-2 mb-6">
            <svg
              className="w-5 h-5 text-[hsl(221,83%,53%)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <h2 className="text-lg font-semibold text-[hsl(222,47%,11%)]">
              Vitals Snapshot
            </h2>
          </div>
          <div className="grid grid-cols-3 gap-12">
            {vitals.map((vital) => (
              <div key={vital.label} className="text-center">
                <p className="text-4xl font-bold text-[hsl(222,47%,11%)] mb-1">
                  {loadingVitals ? "--" : vital.value}
                </p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[hsl(214,32%,91%)] p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link
              ...
              className="text-[hsl(221,83%,53%)] hover:text-[hsl(221,83%,48%)] text-sm font-medium"
            >
              View all
            </Link>
          </div>
          <div className="space-y-3">
            {recentReports.map((report) => (
              <div
                key={report.id}
                className="flex items-center justify-between p-4 rounded-lg bg-[hsl(214,100%,97%)] hover:bg-[hsl(214,32%,91%)] transition-colors cursor-pointer group"
              >
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs px-3 py-1 rounded-full bg-white text-[hsl(215,16%,47%)] font-medium">
                    {report.type}
                  </span>
                  <svg
                    className="w-5 h-5 text-[hsl(215,16%,47%)] group-hover:text-[hsl(222,47%,11%)]"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >

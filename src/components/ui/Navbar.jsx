...
const Navbar = () => {
  ...
  const doctorItems = [
    { to: "/doctor/dashboard", label: "Dashboard", icon: "grid" },
  ];
  ...
  return (
    <nav className={`${isFixed ? "fixed" : "sticky"} top-0 left-0 right-0 z-50 bg-white border-b border-[hsl(214,32%,91%)]`}>
      <div className=" px-4 pr-2 py-3 flex items-center justify-between">
        ...
        {role === "doctor" && (
          <div className="hidden sm:flex items-center gap-1 px-2 py-1 bg-[hsl(221,83%,53%)]/5 rounded-full text-xs text-[hsl(221,83%,53%)] font-medium">
        ...
        <div className="hidden md:flex items-center gap-5 flex-1 justify-start pl-6">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                href={item.to}
                className={`flex items-center gap-2 text-xs font-medium transition-colors whitespace-nowrap no-underline ${
                  isActive(item.to)
                    ? "text-[hsl(221,83%,53%)]"
        ...
        <div className="hidden md:flex items-center gap-3 shrink-0">
          ...
          {role === "patient" && (
            <Link href="/patient/qr" className="no-underline">
              <button className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[hsl(221,83%,53%)] text-white text-xs font-medium hover:bg-[hsl(221,83%,48%)] transition-colors">
          ...
          {(role === "patient" || role === "doctor") && (
            <div className="relative">
              ...
              {showProfileMenu && (
                <div className="absolute right-0 top-12 w-48 bg-white rounded-2xl shadow-lg border border-[hsl(214,32%,91%)] py-3 z-[100] overflow-visible">
                  {role === "doctor" && (
                    <>
                    ...
                    <>
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="w-full flex items-center justify-between px-4 py-3 text-sm text-[hsl(222,47%,11%)] hover:text-[hsl(221,83%,53%)] transition-colors bg-white border-b border-[hsl(214,32%,91%)]"
                        >
                        ...
                        {showLanguageMenu && (
                          <div className="absolute left-full top-0 w-48 bg-white border border-[hsl(214,32%,91%)] rounded-2xl shadow-lg py-2 ml-2 z-[110]">
                            {languages.map((lang) => (
                              <button
                                key={lang}
                                onClick={() => {
                                ...
                                className={`w-full text-left px-4 py-2 text-sm transition-colors ${language === lang ? "text-[hsl(221,83%,53%)] bg-[hsl(221,83%,53%)]/10 rounded-lg mx-2 px-2 w-[calc(100%-16px)]" : "text-[hsl(222,47%,11%)] hover:text-[hsl(221,83%,53%)]"}`}
                              >
                                {lang}
                              </button>
                            ))}
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-4 py-3 text-sm text-[hsl(0,84%,60%)] hover:text-[hsl(0,100%,50%)] transition-colors bg-white border-t border-[hsl(214,32%,91%)]"
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isOpen && (
        <div className="md:hidden absolute top-16 left-0 right-0 bg-white border-b border-[hsl(214,32%,91%)] shadow-lg">
          <div className="px-4 py-3 space-y-1">
            ...
            {role === "patient" && (
              <Link href="/patient/qr" onClick={() => setIsOpen(false)} className="no-underline">
            ...
            {role === "patient" && (
              <div className="pt-3 border-t border-[hsl(214,32%,91%)] mt-3">
                <button
                  onClick={() => setShowLanguageMenu(!showLanguageMenu)}
                  className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-[hsl(222,47%,11%)] hover:text-[hsl(221,83%,53%)] transition-colors"
                >
                  <span className="flex items-center gap-2">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                  ...
                  <svg className={`h-4 w-4 text-[hsl(215,16%,47%)] transition-transform ${showLanguageMenu ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </button>
                {showLanguageMenu && (
                  <div className="bg-white border border-[hsl(214,32%,91%)] rounded-2xl shadow-lg py-2 mt-2">
                    {languages.map((lang) => (
                      <button
                        key={lang}
                        onClick={() => {
            ...
            {(role === "patient" || role === "doctor") && (
              <button
                onClick={handleLogout}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-[hsl(0,84%,60%)] hover:text-[hsl(0,100%,50%)] transition-colors bg-white mt-2 border-t border-[hsl(214,32%,91%)]"
              >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  </svg>
